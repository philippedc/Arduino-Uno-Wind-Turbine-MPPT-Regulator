...
• inverter enable signal : ondulPin  => output 5  blue LED + inverter
...
int VpriMax = 40;                 // MAx value until DumpLoad Resistor security
...
// keep the maximum measured values for display only
    ...
    if( Fpri > kept_FpriMax ) kept_FpriMax = Fpri;
...
void next_window() {
  ...
  window = (window+1) % 14;         // next window modul 10
